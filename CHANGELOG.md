...
- 修复：预创建监听器分批创建，避免数量超过 50（CLB `CreateListener` 单次 `Ports` 上限）导致创建失败。此前仅修复了动态创建监听器场景的分批，本次补齐预创建监听器场景，单 CLB 预创建监听器数量不再受单次 API 限制。
- 修复：端口池扩容逻辑，改为分配端口失败时被动触发扩容。原有基于 `CanAllocate` 的 dry run 只检查能否分配 1 个端口，无法预知下一次分配所需端口数（如单个 Pod 需要 100 个端口），导致端口范围耗尽后无法触发扩容。
- 修复：端口池扩容竞态导致重复创建 CLB。CLB 创建期间（约 10 秒）存在竞态窗口，Binding 持续分配失败会重复触发扩容。改用一次性原子标记吸收竞态请求，CLB 创建后立即加入分配器缓存，既防止重复创建又不阻塞端口真正不足时的合法扩容。
- 修复：显式设置 `endPort`（端口段监听器）时无法扩容 CLB 的问题。
- 修复：`ensureListener` 在 CLB 不存在分支移除 binding 时未清理监听器导致监听器泄露。
...
- 修复：引入腾讯云标签对账，避免自动创建 CLB 时出现 CLB 泄露问题。
- 修复：解决端口池自动创建 CLB 默认放通安全组不生效问题。

## v2.3.7 (2025-08-13)

- 性能优化：绑定 rs 时无需等待任务成功（BatchRegisterTargets 接口为同步任务，无须调用 DescribeTaskStatus 查询任务状态，移除这个查询，每次绑定 rs 时可减少一次 API 调用）。
